...
function breakInfo = breakyaxis(varargin)
    ...
    %Create a sine shaped patch to seperate the 2 sides (THIS SINE SHAPE
        % WAS UGLY SO I REMOVED IT AND REPLACED IT WITH A LINE. TEA)
     breakYLim = [mainPosition(2) mainPosition(2)+mainPosition(4)];
     set(breakAxes,'ylim',breakYLim);
     % theta = linspace(0,2*pi,100);
     theta=linspace(1,0.000001);
...
function annotationAxes = CreateAnnotaionAxes(mainAxes,mainParent)
    ...
    xLabelFont = get(xLabel,'FontSize'); % IT'S NOT WORKING IF I USE THIS SO I MANUALLY SET THE FONT
    yLabel = get(annotationAxes,'YLabel');
    yLabelPosition = get(yLabel,'Position');
    yLabelFont = get(yLabel,'FontSize'); % IT'S NOT WORKING IF I USE THIS SO I MANUALLY SET THE FONT
    title = get(annotationAxes,'Title'); % IT'S NOT WORKING IF I USE THIS SO I MANUALLY SET THE FONT
    ...
    %set(yLabel,'Position',yLabelPosition); % THIS DOES NOT WORK
    set(yLabel, 'Units', 'Normalized', 'Position', [-0.07, 0.5, 0],'FontSize',14); % this works for putting ylabel 
                                                                                   % into the right place and for the font
    set(xLabel,'FontSize',14) % I PUT THAT HERE BECAUSE I NEEDED THIS SIZE IN MY PLOTS AND 
                              % THIS FUNCTION MESSED UP THE FONT AND THE
                              % POSITION OF BOTH OF THE LABELS
