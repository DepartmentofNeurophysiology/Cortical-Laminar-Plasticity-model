...
switch activity 
    ...
    case 0 % do not apply scaling factor
        sf=1;
    otherwise
        disp('Error in providing input for "activity" in ConnMx')
        Mx=[]; % return empty matrix
        return
end%activity 
...
switch vary
    ...
    case 'bouts' % vary bouton number (i.e. synaptic strength)
        Nb=Nb*sf;
    otherwise
        disp('Error in providing input for "vary" in ConnMx')
        Mx=[]; % return empty matrix
        return
end%vary
...
switch layer
    ...
    case 'trans'
    otherwise
        disp('Error in providing input for "layer" in ConnMx')
        Mx=[]; % return empty matrix
end%layer
end%end function
